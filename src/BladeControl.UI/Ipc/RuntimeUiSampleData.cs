...
public static class RuntimeUiSampleData
{
    ...
    public static RuntimeStatusDto Status(
        ...
        string? startRejectionReason = null,
        IReadOnlyList<RuntimeEventDto>? recentEvents = null) => new(
        // Named, for the same reason the production mapper is: this record has runs of
        // same-typed neighbours where a transposition compiles and passes.
        State: state,
        SessionId: sessionId,
        StartTimestamp: sessionId.HasValue ? DateTimeOffset.UtcNow.AddMinutes(-3) : null,
        CurrentProfile: currentProfile,
        CapturedOriginalPerformanceState: sessionId.HasValue ? MachineState() : null,
        CurrentEffectiveFanTargetRpm: effectiveFanTargetRpm,
        LatestAuthoritativeTelemetry: telemetry,
        TelemetryHealth: health,
        Scheduler: scheduler ?? Scheduler(),
        SchedulerHealth: schedulerHealth,
        RuntimeBuild: "0.1.0+sample",
        LastRazerWatchdogState: watchdog,
        LastRazerWatchdogObservedAt: watchdog is null ? null : DateTimeOffset.UtcNow,
        LastFailureReason: lastFailureReason,
        LastStartRejectionReason: startRejectionReason,
