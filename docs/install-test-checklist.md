...
```powershell
Get-FileHash -Algorithm SHA256 .\BladeControl-0.1.6-win-x64.msi
...
```powershell
msiexec /i BladeControl-0.1.6-win-x64.msi /l*v install.log
```
...
- [ ] Appears in Settings → Apps → Installed apps with publisher *BladeControl Project*,
      version *0.1.6*, and a working help link.
...
```powershell
msiexec /x BladeControl-0.1.6-win-x64.msi /l*v uninstall.log
```
